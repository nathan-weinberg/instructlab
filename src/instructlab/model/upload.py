...
class Uploader(abc.ABC):
    """Base class for an uploading backend"""
    ...
    def upload(self):
        ...
        try:
            ...
            if os.path.exists(self.model):
                self.local_model_path = self.model
            else:
            ...
            click.secho(
                f"\nUploading model failed with the following Hugging Face Hub error:\n{DEFAULT_INDENT}{exc}",
                fg="red",
            )
            raise click.exceptions.Exit(1)
...
def upload(ctx, model, model_dir, dest_type, upload_dest, release, hf_token):
    ...
    if dest_type == "hf":
        ...
        click.secho(
            f"\nUploading of type {dest_type} is not yet supported",
            fg="yellow",
        )
    elif dest_type == "s3":
